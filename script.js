...
function setRoleDescription(role) {
  ...
  else if (role === 'منتقم') box.innerText = 'أنت المنتقم 💣: يمكنك استبعاد نفسك وشخص آخر معك وقت التصويت.';
...
window.useAvengerPower = async function () {
  ...
  try {
    ...
    await update(ref(db), updates);
    alert(`تم استخدام قوة المنتقم! تم استبعادك واستبعاد (${targetName}) معك! 💣`);
